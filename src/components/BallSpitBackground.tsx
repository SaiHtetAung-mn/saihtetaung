...
class Z extends InstancedMesh {
  ...
  setColors(colors: number[]) {
    if (Array.isArray(colors) && colors.length > 0) {
      ...
      const materialColor = colorUtils.getColorAt(0);
...
function createBallpit(canvas: HTMLCanvasElement, config: any = {}): CreateBallpitReturn {
  ...
  if (!context) {
    throw new Error('Ballpit background requires WebGL, but no WebGL context is available.');
  }
...
const Ballpit: React.FC<BallpitProps> = ({ className = '', followCursor = true, ...props }) => {
  ...
  useEffect(() => {
    ...
    const frameId = requestAnimationFrame(() => {
      ...
      try {
        ...
        console.warn('Ballpit background disabled:', error);
      }
    });
  ...
  return <canvas className={`block h-full w-full ${className}`.trim()} ref={canvasRef} />;
};
